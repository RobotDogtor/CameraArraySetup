...
%               or in our application. the principal points should be (640, 512)
%               and aspect ratio = 1
%
...
function [Pe,Xe,C,Rot,K] = euclidize(Ws,Lambda,P,X,config)
n = size(Ws,1)/3; % number of points
m = size(Ws,2);	  % number of cameras

% compute B
a = []; b = []; c = [];
for i = 1:n
  a = [a; sum(Ws(3*i-2,:).*Lambda(i,:))];  % summing up the x * lambda for all points
  b = [b; sum(Ws(3*i-1,:).*Lambda(i,:))];  % summing up the y * lambda for all points
  c = [c; sum(Lambda(i,:))];               % summing up the lambda for all points
end
TempA = -P(3:3:3*n, :);
TempB = -P(3:3:3*n, :);
for i = 1:n
