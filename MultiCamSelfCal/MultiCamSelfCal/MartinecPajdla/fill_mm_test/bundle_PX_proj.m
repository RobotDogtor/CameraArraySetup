...
%
% For the bundle adjustment, we are aimming to solve the scale problem, 
% Since we roughly know the distance between each cameras so, here we are
% going to apply regularization to minimize the regularization error
%
%
%
%
...
if ~isfield(opt,'lam_init'), opt.lam_init = 1e-2; end
...
while (nfail < 10) & (stepy*opt.res_scale > opt.max_stepy) & (niter < 100)
