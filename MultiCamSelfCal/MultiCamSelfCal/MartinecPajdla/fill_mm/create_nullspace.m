...
%since the rank of matrix should be 4
...
%every iteration, there will be at least one column that will be added to
%the base collection of nullspace
width                           = 0;
...
for i = 1:num_trials
  ...
  for t = 1:4
    ...
    % c: the chosen column
    % cols: the rest col (total - c)
    cols_chosen = [cols_chosen c];
    % have four random selected columns for each time
    % check just added column
    %return values that in rows and also in c
    rows = intersect(rows, find(I(:,c) > 0));
    % the find the corresponding row based on the selected col
    if t < 4,
