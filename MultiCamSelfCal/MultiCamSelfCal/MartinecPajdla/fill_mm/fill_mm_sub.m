...
%use a specific camaera as cental camera

P=[]; X=[]; lambda=[]; u1=1:m; u2=1:n;   
%u1: sequence of row number
%u2: sequence of col number

%estimate the fundamental matrices and epipoles with the method of [Har95]
 [F,ep,rows,nonrows] = M2Fe(Mfull, central);
 % using the matrix will good row to calculate the fundamental matrix of
 % the central camera and other camera
...
%by using the corresponding point and epipolar geo and fundamental matrix
...
% up to here the entity of in B will be (coordinaate * calculated depth)
