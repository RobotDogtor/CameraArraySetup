...
%    Idepths: refers to whether 1 with entity and 0 NaN
...
for j = 1:n
  full     = find(~isnan(M(1:3:end,j))); % for each column, find the nonNaN entities index for col j
  mis_rows = intersect(find(Idepths(:,j)==0),full); % mis_row, the index that no lambda, no data
  % which means we are not goint to explore any data
  if length(mis_rows) > 0
    submatrix = spread_depths_col(M(k2i(full),j),Idepths(full,j));
    % the we expand the matrix by using 
    ...
    right = submatrix(:,1); % first column
    ...
    if rank(A) < size(A, 2)  % depths cannot be computed => kill the data
      ...
      res   = A \ right;     % solve the linear equation A*res = right
