...
if size(NULLSPACE) == [0 0]
  ...
  r = 4;  % the rank of nullspace should be 4
  [L, S] = nullspace2L(NULLSPACE, r, opt); % compute the based of the nullspace
  %L is the base of the null soace
  clear NULLSPACE;
  
  if isempty(opt.create_nullspace), threshold = 0.01;
  else, threshold = opt.create_nullspace.threshold; end
  if svd_suff_data(S, r, threshold) %passing the singular value and the matrix rank
      % and if the second least sigular value is bigger than threshold
      % then:
    if opt.verbose
      dS = diag(S); %disp(diag(S));  
      % find the 2 least singular values
    ...
    u1 = union(ceil(u1b/3),[]); 
    killb = setdiff(k2i(u1),u1b); 
    if ~isempty(killb), r1b = setdiff(1:3*m,u1b); 
      kill = killb;
...
if size(NULLSPACE,2) < 10*size(NULLSPACE,1) % orig:[A,S,U] = svd(NULLSPACE',0);
   % if the number of column is smaller than 10 times the length of row
  [U,S,V]             = svd(NULLSPACE);
else 
...
Snumcols = size(S,2);  % col of sigma matrix
Snumrows = size(S,1);  % row of sigma matrix
if (Snumrows == 0 | Snumcols + r < Snumrows | Snumrows <= r)
  y = 0;
else
  y = S(Snumrows-r,Snumrows-r) > threshold;  %check if the second last sigular value is bigger than
